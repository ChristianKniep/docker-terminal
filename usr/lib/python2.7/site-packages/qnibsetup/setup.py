...
class QnibSetup(object):
    ...
    def config_dns_srv(self):
        """ discover srv via DNS

        If etcd is not linked but available through dns,
        the current node is configured using the dns target
        """
        self._srv_check = {}
        my_domain = self._cfg.get('defaults', 'domain')
        cmd = "dig SRV +short %s." % my_domain
        proc = envoy.run(cmd)
        for line in proc.std_out.split("\n"):
            try:
                (prio, weight, port, host) = line.split()
            except ValueError:
                continue
            print "> ", prio, weight, port, host
            port = int(port)
            if port in SERVICE_MAP:
                srv_name = SERVICE_MAP[port]['src']
                if srv_name not in ('etcd',):
                    continue
                if srv_name not in self._srv_check:
                    self._srv_check[srv_name] = ""
                if getattr(qnibsetup, "qsetup_%s" % srv_name)(self._cfg, [host,]):
                    self._srv_check['do_sth'] = True
    ...
    def config_local_srv(self):
        ...
        skip = re.compile("(setup|diamond|sshd|syslog\-ng)")
        for srv in os.listdir('/etc/supervisord.d/'):
            ...
            if mat:
                ...
                self._cfg.log.info("SRV_NAME: '%s'" % srv_name)
                if getattr(qnibsetup, "qconfig_%s" % srv_name)(self._cfg):
                    self._srv_check[srv_name] = "restart"
