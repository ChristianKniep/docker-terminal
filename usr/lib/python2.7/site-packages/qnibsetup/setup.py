...
class QnibSetup(object):
    ...
    def config_etcd_srv(self):
        ...
        """
        self._cfg.log.info("# CONFIG_ETCD_SRV")
        ...
        for child in skydns.children:
            ...
            if port in SERVICE_MAP:
                ...
                elif self._cfg.has_option('local', srv_name):
    ...
    def config_local_srv(self):
        ...
        """
        self._cfg.log.info("# CONFIG_LOCAL_SRV")
        # services from docker/terminal
        ## maybe they should be configured as well... like checked or... 
        skip = re.compile("(setup|diamond|sshd)")
        dirs = os.listdir('/etc/supervisord.d/')
        if 'skydns.ini' in dirs:
            new_dirs = ['skydns.ini']
            dirs.remove('skydns.ini')
            new_dirs.extend(dirs)
            del dirs
            dirs = new_dirs
        if 'etcd.ini' in dirs:
            new_dirs = ['etcd.ini']
            dirs.remove('etcd.ini')
            new_dirs.extend(dirs)
            del dirs
            dirs = new_dirs
        for srv in dirs:
    ...
    def check_services(self):
        ...
        """
        self._cfg.log.info("# CHECK_SERVICES")
        for srv, stat in self._srv_check.items():
            if stat == 'restart':
                qnibsetup.restart_supervisor(self._cfg, srv)
            elif stat == 'check':
                qnibsetup.check_supervisor(self._cfg, srv)

    def eval_services(self):
        """ iterates over services and checks for changes
        """
        self._cfg.log.info("# EVAL_SERVICES")
        sec = 'services'
    ...
    def read_local_dns_server(self):
        ...
        """
        self._cfg.log.info("# READ_LOCAL_DNS_SERVER")
    ...
    def eval_linked_containers(self):
        """ does sth
        """
        self._cfg.log.info("# EVAL_LINKED_CONTAINERS")
        services = {}
    ...
    def add_service(self, srv_name, ip_list):
        """ adds a service
        """
        self._cfg.log.info("## ADD_SERVICE('%s', '%s'" % (srv_name, ",".join(ip_list)))
