...
def qsetup_carbon(cfg, srv_hosts):
    ...
    if handler_cfg['host'] != carbon_target:
        cfg.log.info("Overwrite host '%s' by '%s'" % (handler_cfg['host'], carbon_target))
        handler_cfg['host'] = carbon_target
...
def qsetup_etcd(cfg, srv_hosts):
    ...
    if len(opts) > 1:
        cfg.log.warn("More then one local service, only the last will appear as SRV record '%s'" % opts)
    for opt in opts:
        port = cfg.get('local', opt)
        rec_vals["a"] = '{"Port":%s, "Host":"%s"}' % (port, my_ipv4)
    for srv_host in srv_hosts:
