...
def qconfig_syslog_ng(cfg):
    ...
    """
    if cfg.has_option('local', 'etcd'):
        client = etcd.Client(host="127.0.0.1")
    else:
        client = etcd.Client(host="etcd.qnib")
    try:
        log_str = client.read('/skydns/qnib/logstash')
    except KeyError:
        return False
    log_str = log_str.value.replace("'", "\"")
    log_dic = json.loads(log_str)
    return add_syslog_fwd(log_dic['host'], log_dic['port'])

def qconfig_graphite_web(cfg):
    """ Check if graphite-web's targets are correct
    """
    cfg.update_cfg('local', 'graphite-web', 80)

def qconfig_graphite_api(cfg):
    """ Check if graphite-web's targets are correct
    """
    cfg.update_cfg('local', 'graphite-api', 80)
...
def qconfig_carbon(cfg):
    """ Do we need to set something up?
    """
    cfg.update_cfg('local', 'carbon-plain', 2003)
    cfg.update_cfg('local', 'carbon-pickle', 2004)
    cfg.update_cfg('local', 'carbon-link', 7002)
...
def qconfig_etcd(cfg):
    ...
    ### IF we are not the DNS, we must run etcd
    if cfg.get('defaults', 'docker_dns') == 'False':
        check_supervisor(cfg, 'etcd')
    etcd_lst = []
    if cfg.has_option('services', 'etcd'):
...
def qsetup_etcd(cfg, srv_hosts=None):
    """ setup the etcd service
    """
    if srv_hosts is None:
        srv_hosts = ['etcd.qnib',]
    my_ipv4 = netifaces.ifaddresses('eth0')[netifaces.AF_INET][0]['addr']
    my_domain = cfg.get('defaults', 'domain')
...
def check_supervisor(cfg, srv_name):
    ...
    if not re.match("%s\s+RUNNING" % srv_name, std_out):
        restart_supervisor(cfg, srv_name)
        cfg.log.info("supervisor service '%s' restarted" % srv_name)
    else:
        cfg.log.info("supervisor service '%s' already running" % srv_name)

def restart_supervisor(cfg, srv_name):
    ...
    for line in proc.std_out.split('\n'):
        if re.match("%s\: started" % srv_name, line):
            cfg.log.info('restart successful')
            time.sleep(2)
            break
    else:
